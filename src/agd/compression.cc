...
Status resize_output(z_stream &strm, vector<char> &output, size_t extend_len) {
  ...
  if (strm.avail_out == 0) {
    ...
    if (output.capacity() < new_cap) {
      ...
      strm.next_out =
          reinterpret_cast<unsigned char *>(&output[strm.total_out]);
...
Status decompressGZIP(const char *segment, const std::size_t segment_size,
  ...
  // TODO do we need to call inflate in while lopp while status == Z_OK (like
  // deflate)?
      ...
      while (status != Z_STREAM_END && s.ok() && strm.total_in < segment_size) {
        ...
        switch (status) {
          ...
          case Z_STREAM_END:
            break;
          default:  // an error
            s = Internal("inflate(Z_NO_FLUSH) returned code ", status,
                         " with message '", strm.msg == NULL ? "" : strm.msg,
                         "'");
            break;
        }
      }
    } else {
      s = Internal("inflate(Z_FINISH) return code ", status, " with message '",
  ...
  if (status != Z_OK) {  // s.ok() status to make sure we don't override
                         // non-inflateEnd error
    if (s.ok()) {
...
Status decompressGZIP(const char *segment, const size_t segment_size,
                      vector<char> &output) {
  // TODO this only supports decompress write, not appending
  // this is an easy change to make, but requires some more "math"
...
Status compressGZIP(const char *segment, const size_t segment_size,
                    vector<char> &output) {
  ...
  if (status != Z_OK) {
    return Internal("deflateInit() didn't return Z_OK. Return ", status,
                    " with 2nd param ", Z_DEFAULT_COMPRESSION);
  }
  ...
  auto total_out =
      strm.total_out;  // save it here because deflate nukes this value
...
AppendingGZIPCompressor::~AppendingGZIPCompressor() { finish(); }

AppendingGZIPCompressor::AppendingGZIPCompressor(Buffer &output)
    : output_(output) {}
...
void AppendingGZIPCompressor::ensure_extend_capacity(size_t capacity) {
  ...
  stream_.next_out = const_cast<unsigned char *>(
      reinterpret_cast<const unsigned char *>(&output_[sz]));
}
}  // namespace agd
