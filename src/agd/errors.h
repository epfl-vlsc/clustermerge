...
#define AGD_PREDICT_TRUE(condition) __builtin_expect(static_cast<bool>(condition), 1)
#define AGD_PREDICT_FALSE(condition) __builtin_expect(static_cast<bool>(condition), 0)
...
void AppendToMessage(::agd::Status* status, Args... args) {
  *status = ::agd::Status(
      status->code(),
...
#define AGD_RETURN_IF_ERROR(...)                          \
  do {                                                   \
    const ::agd::Status _status = (__VA_ARGS__);  \
    if (AGD_PREDICT_FALSE(!_status.ok())) return _status; \
  } while (0)

#define AGD_RETURN_WITH_CONTEXT_IF_ERROR(expr, ...)                  \
  do {                                                              \
    ::agd::Status _status = (expr);                          \
    if (AGD_PREDICT_FALSE(!_status.ok())) {                          \
      ::agd::errors::AppendToMessage(&_status, __VA_ARGS__); \
      return _status;                                               \
    }                                                               \
  } while (0)
...
#define DECLARE_ERROR(FUNC, CONST)                                       \
  template <typename... Args>                                            \
  ::agd::Status FUNC(Args... args) {                              \
    return ::agd::Status(                                         \
        ::agd::errors::CONST,                                      \
        absl::StrCat(                                   \
            ::agd::errors::internal::PrepareForStrCat(args)...)); \
  }                                                                      \
  inline bool Is##FUNC(const ::agd::Status& status) {             \
    return status.code() == ::agd::errors::CONST;                  \
  }
