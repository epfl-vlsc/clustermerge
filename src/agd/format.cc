...
struct BaseMap {
  ...
  constexpr BaseMap(BaseAlphabet base_, char base_char_)
      : base(base_), base_char(base_char_) {}
};
...
Status append(const BinaryBases *bases, const std::size_t record_size_in_bytes,
              Buffer &data, Buffer &lengths) {
  if (record_size_in_bytes % sizeof(uint64_t) != 0) {
    return InvalidArgument("Size of record ", record_size_in_bytes,
                           " is not a multiple of ", sizeof(uint64_t));
  }
  ...
  for (size_t i = 0; i < record_size_in_base_records; ++i) {
    ...
    AGD_RETURN_IF_ERROR(base->append(data, &length));
  }
  RelativeIndex char_len = static_cast<RelativeIndex>(length);
  lengths.AppendBuffer(reinterpret_cast<const char *>(&char_len),
                       sizeof(char_len));
  return Status::OK();
}
...
Status BinaryBases::append(Buffer &data, size_t *num_bases) const {
  ...
  for (size_t i = 0; i < compression;) {
    ...
    if (res == nullptr) {
      return Internal("unable to convert value ", bases_copy & 0x1ff,
                      " to a triple\n");
    }
...
Status BinaryBases::getBase(const size_t position, char *base) const {
...
Status IntoBases(const char *fastq_base, const std::size_t fastq_base_size,
                 std::vector<BinaryBases> &bases, bool warning) {
  bases.clear();
  size_t base_idx = 0;
  BinaryBases bb;
  for (size_t i = 0; i < fastq_base_size; i++) {
    if (fastq_base[i] ==
      ...
      continue;
    AGD_RETURN_IF_ERROR(bb.setBase(fastq_base[i], base_idx++, warning));
...
Status BinaryBases::setBase(const char base, size_t position, bool warning) {
  ...
  switch (base) {
    ...
    default:
      if (warning) {
        b = BaseAlphabet::N;
        // LOG(INFO) << "Warning: base " << string(&base, 1) << " is not
        // compatible and will be changed to 'N'.";
      } else {
        return InvalidArgument(
            "Unable to convert the following base character: ",
            string(&base, 1), " are you sure this column is base pair data?");
      }
  }
...
Status BinaryBases::setBaseAtPosition(const BaseAlphabet base,
                                      const size_t position) {
  assert(position < compression);
  if (position < compression) {
    uint64_t packed_base =
        0x7ull & static_cast<uint64_t>(base);  // get the lower bits only!
