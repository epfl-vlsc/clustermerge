...
void debug_alignment(char* name, void* profile, const char* s2, int ls2,
  ...
  printf(
      "Aligning %s: profile = %lu, s2 = %s, len(s2) = %d, gapOpen = %f, gapExt "
      "= %f, threshold = %f\n",
...
void debug_profile(char* name, void* pb, const char* q, int queryLen,
  ...
  printf(
      "Successfully created %s profile at %lu from query = %s, len(query) = "
      "%d\n",
      name, (uint64_t)pb, query, queryLen);
...
const double ProteinAligner::pam_list_[] = {35,  49,  71,  98,  115, 133,
                                            152, 174, 200, 229, 262, 300};

void ProteinAligner::FindStartingPoint(const char* seq1, const char* seq2,
  ...
  /*ofstream file;
  file.open(string("dump/fsp_alignment_") + to_string(filenum) +
  string(".csv")); filenum++; file << seq1_len << ", " << seq2_len << "\n";*/
  ...
  for (const double& pam : pam_list_) {
    ...
    /*file << alignment.seq1_min << ", " << alignment.seq1_max << ", " <<
      alignment.seq2_min
      << ", " << alignment.seq2_max << "\n";*/
...
agd::Status ProteinAligner::AlignSingle(const char* seq1, const char* seq2,
                                        int seq1_len, int seq2_len,
                                        Alignment& result) {
  agd::Status s;
...
agd::Status ProteinAligner::AlignLocal(const char* seq1, const char* seq2,
                                       int seq1_len, int seq2_len,
                                       Alignment& result) {
  // find starting point
  ...
  while (true) {
    ...
    if (point.alignment.env ==
        &new_env)  // envs are basically static, so this is safe
      break;
  ...
  result.seq1_length =
      point.seq1_len - CountUnderscore(point.seq1, point.seq1_len);
  result.seq2_length =
      point.seq2_len - CountUnderscore(point.seq2, point.seq2_len);

  return agd::Status::OK();
}

agd::Status ProteinAligner::AlignDouble(const char* seq1, const char* seq2,
                                        int seq1_len, int seq2_len,
  ...
  // LOG(INFO) << "aln dbl score " << score << ", max1, max2: " << max1 << ", "
  // << max2;
  ...
  // LOG(INFO) << "rev 1 is " << PrintNormalizedProtein(seq1_rev.c_str(),
  // seq1_rev.length()); LOG(INFO) << "rev 2 is " <<
  // PrintNormalizedProtein(seq2_rev.c_str(), seq2_rev.length());

  ProfileDouble* profile_rev =
      createProfileDoubleSSE(seq1_rev.c_str(), seq1_rev.length(), env.matrix);
  ...
  // LOG(INFO) << "aln dbl rev score " << score_rev << ", max1, max2: " <<
  // max1_rev << ", " << max2_rev;
  ...
  // LOG(INFO) << "seq1max: " << max1 << ", seq2max: " << max2 << ", seq1min: "
  // << result.seq1_min
...
double ProteinAligner::c_align_double_global(double* matrix, const char* s1,
  ...
  for (i = 1; i <= ls1; i++) {
    ...
    for (j = 1; j <= ls2; j++) {
      ...
      if (rowdel < t) rowdel = t;

      /* TODO: check that det vs prob can never be a Self */
      t = NoSelf /*&& ProbSeqCnt == 0*/ && s1 + i == s2 + j
              ? MINUSINF
              : Sj1 + Score_s1[AToInts2[j]];
      if (t < rowdel) t = rowdel;
      if (t < data->coldel[j]) t = data->coldel[j];
       ...
       *                                                                   }
       *                                                                         }
       * else if (mode == Shake && t > MaxScore) { MaxScore = t; *Max1 = i;
       *                                                                                           *Max2
       * = j; if (MaxScore >= goal) { totcells += j; return (MaxScore);
       *                                                                                                                   }
       *                                                                                                                         }*/
      data->S[j] = t;
      Sj1 = t2;
...
int ProteinAligner::AlignStrings(double* matrix, char* s1, int len1, char* s2,
  ...
  if (len1 == len2 && s1 != s2) {
    for (i = tot = 0; i < len1; i++)
      tot += /*DMScore(s1[i],s2[i],matrix)*/ matrix[s1[i] * MATRIX_DIM + s2[i]];
    if (tot >= escore) {
      for (i = 0; i < len1; i++) {
        o1[i] = s1[i];
        o2[i] = s2[i];
  ...
  /* splitting on a vertical deletion */
  {
    ...
    len = AlignStrings(matrix, s1, i3, s2, i,
                       coldel1[i] - gap_open - gap_ext * (i1 - i3 - 1), o1, o2,
                       0.0, gap_open, gap_ext, data);
    for (j = i3 + 1; j < i4; j++) {
      o1[len] = s1[j - 1];
      o2[len++] = '_';
    }
